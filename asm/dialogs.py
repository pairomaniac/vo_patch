...
ITEMS = [
    ...
    ('Scorekeeping', 0x9c67, 124, 44, 72, 14, PUSH,     None),
    ('Quit Program', 0x9c41,  16, 84, 72, 14, PUSH,     None),
    ('Close',      IDCANCEL, 146, 84, 50, 14, PUSH,     None),
]
...
def build_extras():
    ...
    tpl += struct.pack('<5H', len(ITEMS), 0, 0, 212, 104)
    ...
    inc = ''.join('%%define %-11s 0x%08x\n' % (name, value) for name, value in (
        ...
        ('RATES', rates),
        ('TEMPLATE', TEMPLATE),
        ('CMD_QUIT', dict((l, i) for l, i, *_r in ITEMS)['Quit Program']),
        ('IDCANCEL', IDCANCEL),
    ))
