...
| **Credit the patch in the ending roll** | `0x2bbb54`, `scrstfcg.bin`, `scrstfmp.bin` | the roll is a list of blocks, 12 bytes each as (flag, width, height) in cells, read from `0x6bcd48` and centred on 51 cells by `0x448e86`; four entries are spliced in after the title block and the rest shift down, the cells go into `scrstfmp.bin` at the same point, and the tiles on the end of `scrstfcg.bin`, whose indices the loader rebases at `0x483d9d` |
