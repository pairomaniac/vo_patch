...
| **Fix crash on ALT+TAB** | `0x1c4aa2`, `0x1c5726`, `0x1c5412` | the surface recreate and `setactive` hooked at their entries, the loop's idle call wrapped: no resume without surfaces, the recreate retried until it takes |
...
The `WM_ACTIVATEAPP` handler at `0x5c6d2d`, on activation, calls
`0x5c56a2` to release both DirectDraw surfaces and create them again -
`SetDisplayMode`, then the primary and its attached back buffer - and
resumes the frame loop through `GRESUME` without looking at the result.
`0x5c56a2` returns 0 with the pointers left null if any step fails, which
on a DirectDraw that cannot hand exclusive mode back at that instant is
what happens: Wine's own, the window not yet foreground, during the intro
movie. The next frame then locks a null back buffer at `0x5c8103`, and if
that were guarded would flip a null primary at `0x5c650b`, and then reach
DirectSound through a null interface at `0x58a244`; every other user of
the surfaces checks for null first, and the frame path does not.

The patch is `asm/activate.asm`, three pieces, each at the entry of the
function itself so that every caller is covered. `0x5c56a2` is hooked at
its entry with the caller's return address swapped for the stub's, so the
result comes past it: on failure it sets the inactive flag `0x1add128`
itself - the intro movie's deactivation does not pause the game, so
nothing else would, and the handler is not the only caller: a second
release-and-create at `0x5b1510` runs from the title state and two more
window-procedure branches. `setactive` (`0x5c6326`, the pause on 1 and the resume on 0 that
...
idles on it, so waiting costs nothing but the frames that had nothing to
draw on. cnc-ddraw never fails the create and never saw any of this.
