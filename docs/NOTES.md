...
| **Credit the patch in the ending roll** | `0x1fcec8`, `0x1fcecc`, `0x2bbb54`, `scrstfcg.bin`, `scrstfmp.bin` | the roll is a list of blocks, 12 bytes each as (flag, width, height) in cells, read from `0x6bcd48` and centred on 51 cells by `0x448e86`; four entries are spliced in after the title block and the rest shift down, the cells go into `scrstfmp.bin` at the same point, and the tiles on the end of `scrstfcg.bin`, whose indices the loader rebases at `0x483d9d`; the loader reads both files to byte counts held at `0x5fdac8` and `0x5fdacc` rather than to their size, so the two constants grow with them |
